...
const Hero = () => {
  return (
    <section className="relative w-full min-h-screen flex flex-col items-center justify-center overflow-hidden bg-[#1a0505]">
      ...
      <div className="absolute inset-0 w-full h-full z-0">
          <Suspense fallback={null}>
            <Canvas camera={{ position: [0, 0, 6], fov: 45 }} gl={{ antialias: true, alpha: true }}>
              {/* Dimmed ambient light so the beams pop more */}
              <ambientLight intensity={0.1} />
              <spotLight position={[10, 10, 10]} angle={0.5} penumbra={1} intensity={1} color="#f59e0b" />
              {/* Darker environment to keep contrast high */}
      ...
      {/* --- CONTRAST FIX: Radial Gradient Overlay --- */}
      {/* This sits between the 3D canvas and the Text. It darkens the center slightly. */}
      ...
      <div className="relative z-10 flex flex-col items-center justify-center max-w-5xl px-6 space-y-8 text-center mt-12">
        ...
        {/* Main Title - Increased Drop Shadow for Contrast */}
        ...
        <div className="flex flex-col sm:flex-row gap-6 justify-center pt-8 w-full">
          <button className="px-8 py-4 rounded-sm bg-gradient-to-r from-amber-600 to-amber-800 text-white font-merriweather font-bold tracking-wider flex items-center justify-center gap-2 hover:scale-105 shadow-[0_0_20px_rgba(217,119,6,0.5)] transition-all duration-300 border border-amber-500/50 z-20">
            <Sparkles size={18} />
            GET PASSES
          </button>
          
          <LiquidGlass className="px-8 py-4 flex items-center justify-center gap-2 cursor-pointer hover:bg-black/40 text-amber-100 font-merriweather border-amber-500/30 z-20">
